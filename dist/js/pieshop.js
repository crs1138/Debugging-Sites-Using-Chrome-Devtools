...
function loadAllPies() {
    ...
    fetch("/api/pies").then(response=>response.json()).then(data=>{
...
function loadNutritionInfo() {
    ...
    fetch("/api/nutrition").then(response=>response.json()).then(data=>console.log(data)).catch(err=>console.error("There was a problem retrieving the nutritional information."));
